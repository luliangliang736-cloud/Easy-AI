...
async function generateVariationRowsWithLLM(records, instruction = "") {
  ...
  const systemPrompt = [
    ...
    "6. 如果用户要求保持原版/沿用模板/风格不变/只换文案（对全部或某些条目），对应条目的「风格」必须以“保持原版”开头（这是系统的原版锁定触发词，会锁定背景/配色/背板/装饰），后面可以附加用户要求的小改说明（例如“保持原版，姿势更活泼”）；不要自行发挥新的配色或风格方向。此时如果用户同时要求服装/姿势按文案调整，把服装变化写进「服装」字段即可，服装和姿势不受锁定影响。",
